...
![image-20250522211123367](imgs/image-20250522211123367.png)
...
![image-20250522211301009](imgs/image-20250522211301009.png)
...
![image-20250522212642410](imgs/image-20250522212642410.png)
...
![image-20250522212900491](imgs/image-20250522212900491.png)

并发（concurrent）是某个时间段轮流做多件事情

并行（parallel）是某一时刻做多件事情

![image-20250522213107806](imgs/image-20250522213107806.png)
...
在`t2`中调用`t1.join()`，在`t3`中调用`t2.join()`最终无论你先启动哪个线程都会按照``t1 -> t2 -> t3`的顺序执行。`t1.join()`需要在`t1.start()`后才能生效，`t2`也是，每个线程都是如此。
...
![image-20230504165342501](imgs/image-20230504165342501.png)
...
![image-20230504165833809](imgs/image-20230504165833809.png)
...
![image-20230504172253826](imgs/image-20230504172253826.png)

##### MarkWord

在不同的锁状态下，MarkWord的结构是不一样的：

![image-20230504172541922](imgs/image-20230504172541922.png)
...
在 没有任何线程持有锁的情况下，锁对象处于无锁状态（Unlocked），此时任何线程都可以尝试获取这把锁，也就是尝试修改（CAS 或加锁）它的状态。但能否成功获取锁，还取决于是否竞争成功。

##### 偏向锁

当一个线程第一次进入同步块时，JVM 会将该线程的 ID 记录在对象头（Mark Word）中，表示该对象的锁偏向于该线程。同时，在线程的栈帧中会记录锁的信息（锁记录）。此后，若同一线程再次进入该同步块，只需检查对象头中的线程 ID 是否为自己，若是，则说明锁已偏向当前线程，线程可以直接进入同步块，无需 CAS 操作，从而大幅提升加锁效率。

如果有其他线程尝试获取该偏向锁，JVM 会进行以下处理：

**检查偏向线程是否存活：**

- 如果偏向线程不再活跃（如已退出或长时间未参与竞争），JVM 会尝试使用 CAS 将对象头中的线程 ID 替换为当前线程的 ID，从而完成偏向锁的转移，锁仍然处于偏向状态；
- 如果偏向线程仍然活跃，JVM 会发起偏向锁的撤销流程：

**撤销偏向锁并升级：**

- 在安全点（SafePoint）暂停偏向线程或等待其达到安全点；
- 将对象的 Mark Word 中的偏向标志位置为 0；
- 将锁标志位设置为 `00`，表示升级为轻量级锁；
- 后续锁的获取将通过轻量级锁的机制进行，即通过 CAS 竞争来加锁。

偏向锁在没有竞争的情况下几乎没有加锁开销，一旦发生竞争会自动升级为轻量级锁，并在必要时进一步升级为重量级锁（monitor）。

```mermaid
graph TD
  A[线程第一次进入同步块] --> B[Mark Word记录线程ID和偏向状态]
  B --> C{线程再次进入}
  C -- 相同线程ID --> D[直接进入 无需CAS]
  C -- 不同线程ID --> E[检查原线程是否活跃]
  E -- 不活跃 --> F[CAS替换线程ID 仍为偏向锁]
  E -- 活跃 --> G[撤销偏向锁]
  G --> H[等待原线程到达安全点]
  H --> I[清除偏向标志 升级为轻量级锁]
  I --> J[CAS竞争进入轻量级锁流程]
...
多个线程在不同时段获取同一把锁，无并发竞争或仅有短暂重叠，即几乎不存在锁竞争的情况。针对这种情况，JVM 采用轻量级锁来避免线程的阻塞与唤醒。

**加锁的流程**

在线程栈中创建一个Lock Record，将其obj指针指向锁对象。

![image-20230504173520412](imgs/image-20230504173520412.png)

通过CAS指令将Lock Record的地址存储在对象头的mark word中，将对象头的mark word原本记录的信息存储进Lock Record中（数据进行交换），如果对象处于无锁状态则可以修改成功，代表该线程获得了轻量级锁。

![image-20230504173611219](imgs/image-20230504173611219.png)

如果是当前线程已经持有该锁了，代表这是一次锁重入。在栈帧中新增一个Lock Record，设置Lock Record的mark word部分为null，起到了一个重入次数计数器的作用。

![image-20230504173922343](imgs/image-20230504173922343.png)

如果CAS修改失败，且该线程又不是重入线程，说明发生了锁竞争，需要膨胀为重量级锁。

在轻量级锁中，每次加锁都需要cas，不管是否重入，而偏向锁只需要在重入的情况下，只需要在第一次加锁时执行cas。

**解锁过程**

遍历线程的栈帧，找到第一个obj指针指向的是当前锁对象的Lock Record。

如果Lock Record的Mark Word为null，代表这是一次重入计数，将obj指针设置为null后continue。

![image-20230504173955680](imgs/image-20230504173955680.png)

如果Lock Record的 Mark Word不为null，则利用CAS指令将对象头的mark word恢复成为无锁状态。如果失败则膨胀为重量级锁，以重量级锁的方式释放锁。

![image-20230504174045458](imgs/image-20230504174045458.png)

##### 再谈Monitor重量级锁

当多个线程竞争同一把锁时，JVM 会先尝试使用轻量级锁的方式通过 CAS 加锁。如果 CAS 失败，说明存在锁竞争。此时，JVM 并不会立刻膨胀为重量级锁，而是先让当前线程**自旋一段时间**，期望持锁线程很快释放锁，从而避免线程挂起带来的性能损耗。

这个自旋阈值是**自适应的**，JVM 会根据历史自旋成功与否的情况动态调整自旋阈值。如果线程一直自旋等待锁的释放，而迟迟未能获取锁，就会进入一种“忙等”状态，白白占用 CPU 资源，造成性能浪费，因此需要设置阈值。

如果自旋等待后仍无法获得锁，JVM 认为轻量级锁无法满足当前竞争需求，就会将锁**升级为重量级锁**，线程也会被挂起并加入 monitor 的等待队列，直到被唤醒。

前面**Monitor重量级锁**部分有一个遗留问题，Monitor和锁对象是如何关联的？

每个 Java 对象都可以关联一个 Monitor 对象，锁对象进入重量级锁状态后，**该对象头的Mark Word 中就会被设置指向 Monitor 对象的指针**，让对象与monitor产生关联。

![image-20230504172957271](imgs/image-20230504172957271.png)
...
![image-20230504181638237](imgs/image-20230504181638237.png)
...
![image-20230504181947319](imgs/image-20230504181947319.png)
...
![image-20230504182129820](imgs/image-20230504182129820.png)

- 线程2操作：V：int a = 100，A：int a = 100，B：修改后的值：int a = 99(a--)
  - 线程2拿A的值与主内存V的值进行比较，判断是否相等(目前不相等，因为线程1已更新V的值99)
  - 不相等，则线程2更新失败

![image-20230504181827330](imgs/image-20230504181827330.png)
...
![image-20230504182447552](imgs/image-20230504182447552.png)
...
![image-20230504182737931](imgs/image-20230504182737931.png)

都是native修饰的方法，由系统提供的接口执行，并非java代码实现，一般的思路也都是自旋锁实现

![image-20230504182838426](imgs/image-20230504182838426.png)

在java中比较常见使用有很多，比如ReentrantLock和Atomic开头的线程安全类，都调用了Unsafe中的方法

- ReentrantLock中的一段CAS代码

![image-20230504182958703](imgs/image-20230504182958703.png)
...
- 主线程将 `running` 设置为 `false`，但子线程**可能永远不会看到这个修改**，导致无限循环。
...
![image-20230505082441116](imgs/image-20230505082441116.png)
...
![image-20230505082835588](imgs/image-20230505082835588.png)

屏障添加的示意图

![image-20230505082923729](imgs/image-20230505082923729.png)
...
![image-20230505083124159](imgs/image-20230505083124159.png)

屏障添加的示意图

![image-20230505083217904](imgs/image-20230505083217904.png)
...
![image-20230505083840046](imgs/image-20230505083840046.png)
...
![image-20230505084451193](imgs/image-20230505084451193.png)
...
![image-20230505091736569](imgs/image-20230505091736569.png)
...
![image-20230505091827720](imgs/image-20230505091827720.png)
...
![image-20230505092151244](imgs/image-20230505092151244.png)

而Sync的父类是AQS，所以可以得出ReentrantLock底层主要实现就是基于AQS来实现的

![image-20230505091833629](imgs/image-20230505091833629.png)

**工作流程**

![image-20230505092340431](imgs/image-20230505092340431.png)
...
![image-20220902171032898](imgs/image-20220902171032898.png)
...
![image-20220902171426738](imgs/image-20220902171426738.png)

第二：使用jstack查看线程运行的情况，下图是截图的关键信息

运行命令：`jstack -l 46032`

![image-20220902172229567](imgs/image-20220902172229567.png)
...
![image-20230505092654811](imgs/image-20230505092654811.png)
...
![image-20230505093055382](imgs/image-20230505093055382.png)
...
![image-20230505093507265](imgs/image-20230505093507265.png)
...
![image-20230505205200628](imgs/image-20230505205200628.png)
...
![image-20230505210853493](imgs/image-20230505210853493.png)
...
![image-20230505211002252](imgs/image-20230505211002252.png)
...
![image-20230505211209336](imgs/image-20230505211209336.png)
...
![image-20230505220514872](imgs/image-20230505220514872.png)
...
![image-20230505220701835](imgs/image-20230505220701835.png)
...
![image-20230505221424359](imgs/image-20230505221424359.png)
...
![image-20230505221837189](imgs/image-20230505221837189.png)
...
![image-20230505221959259](imgs/image-20230505221959259.png)
...
![image-20230505222050294](imgs/image-20230505222050294.png)
...
![image-20230505222126391](imgs/image-20230505222126391.png)
...
![image-20230505222203615](imgs/image-20230505222203615.png)
...
![image-20220821003816845](imgs/image-20220821003816845.png)
...
![image-20230505223014946](imgs/image-20230505223014946.png)
...
![image-20230505223219951](imgs/image-20230505223219951.png)

详细实现流程：

![image-20230505223246059](imgs/image-20230505223246059.png)

#### 案例二（数据汇总）

在一个电商网站中，用户下单之后，需要查询数据，数据包含了三部分：订单信息、包含的商品、物流信息；这三块信息都在不同的微服务中进行实现的，我们如何完成这个业务呢？

![image-20230505223442924](imgs/image-20230505223442924.png)

- 在实际开发的过程中，难免需要调用多个接口来汇总数据，如果所有接口（或部分接口）的没有依赖关系，就可以使用线程池+future来提升性能

- 报表汇总

  ![image-20230505223536657](imgs/image-20230505223536657.png)



#### 案例三（异步调用）

![image-20230505223640038](imgs/image-20230505223640038.png)
...
![image-20230505224057228](imgs/image-20230505224057228.png)
...
![image-20230505224341410](imgs/image-20230505224341410.png)
...
![image-20230505224626253](imgs/image-20230505224626253.png)

**get方法/remove方法**

![image-20230505224715087](imgs/image-20230505224715087.png)
...
![image-20230505224755797](imgs/image-20230505224755797.png)

- 弱引用：表示一个对象处于可能有用且非必须的状态。在GC线程扫描内存区域时，一旦发现弱引用，就会回收到弱引用相关联的对象。对于弱引用的回收，无关内存区域是否足够，一旦发现则会被回收

![image-20230505224812015](imgs/image-20230505224812015.png)

每一个Thread维护一个ThreadLocalMap，在ThreadLocalMap中的Entry对象继承了WeakReference。其中key为使用弱引用的ThreadLocal实例，value为线程变量的副本

![image-20230505224857538](imgs/image-20230505224857538.png)
