...
![image-20230506094254360](imgs/image-20230506094254360.png)

**JVM由哪些部分组成，运行流程是什么？**

![image-20230506094411247](imgs/image-20230506094411247.png)
...
- 类加载器（ClassLoader）将编译器生成的字节码文件（`.class`）加载到运行数据区中的方法区，为程序运行做准备。
- 字节码是一种平台无关的中间表示形式，不能被底层系统直接执行。JVM 的执行引擎会将其解释或即时编译为机器码，交由 CPU 执行。
- 当程序需要访问底层资源时，JVM 会通过 JNI（Java Native Interface）调用本地方法库，完成文件、网络等操作系统级别的任务。
...
程序计数器是线程私有的内存区域，用于记录当前线程所执行到的字节码指令地址。它是支持 JVM 多线程切换的关键组件，可以在线程切换时准确恢复执行位置。

>javap -verbose  xx.class    打印堆栈大小，局部变量的数量和方法的参数。

![image-20230506094602329](imgs/image-20230506094602329.png)



Java 虚拟机支持多线程并发执行，但线程的实际调度由操作系统控制，通常采用时间片轮转的方式。在单核处理器上，同一时刻只能执行一个线程，操作系统会不断切换线程，制造出多个线程“同时运行”的假象。

那么，当一个线程被挂起后，JVM 如何知道它上一次执行到哪？这正是程序计数器（PC Register）的作用——它是每个线程私有的内存区域，专门用于记录当前正在执行的字节码指令地址。当线程恢复运行时，JVM 通过程序计数器读取上一次的位置，从而实现线程从中断处继续执行。

值得注意的是，程序计数器是 JVM 规范中**唯一不会发生 OOM（内存溢出）**的区域，它不参与垃圾回收，空间占用极小。
...
![image-20230506094803545](imgs/image-20230506094803545.png)
...
为了避免**永久代/方法区**出现OOM，所以在java8中将堆上的**永久代/方法区**给移动到了本地内存上，重新开辟了一块空间，叫做**元空间**。那么现在就可以避免掉OOM的出现了。

![image-20230506094938843](imgs/image-20230506094938843.png)
...
那么，Java 8 中 PermGen（永久代）为什么被移出 HotSpot JVM 了？
...
Java7 之后，PermGen 区中的字符串常量池被移到了堆内存中 ，Java 8 时，PermGen 被元空间代替，其他内容比如**类元信息、字段、静态属性、方法、常量**等都移动到元空间区。比如 java/lang/Object 类元信息、静态属性 System.out、整型常量等。
...
- 每个线程运行时所需要的内存结构，称为虚拟机栈，先进后出
- 每个栈由多个栈帧（frame）组成，对应着每次方法调用时所占用的内存
- 每次调用执行一个方法时压入一个栈帧，方法执行结束时弹出栈帧
- 每个线程只能有一个活动栈帧，对应着当前正在执行的那个方法

![image-20230506095140595](imgs/image-20230506095140595.png)
...
未必，默认的线程栈大小通常为 1024KB（1MB）。

更大的栈虽然能容忍更深的递归，但会在虚拟地址空间层面占用更多“预留”区域，导致可创建的线程数减少。

如果线程栈设置得过大，会占用更多内存，从而减少系统能创建的线程总数。例如，假设 JVM 可用于线程栈的内存总量为 512MB，每个线程使用 1MB 栈空间，那么理论上最多可以创建约 512 个线程；如果将栈大小设置为 2048KB（2MB），则最多只能创建约 256 个线程，那么活动的栈帧就会减半。
...
- 比如以下代码：
  ...
  ![image-20230506095306061](imgs/image-20230506095306061.png)

**栈内存溢出情况**

- 栈帧过多导致栈内存溢出，典型问题：递归调用

  ![image-20230506095401637](imgs/image-20230506095401637.png)
...
![image-20230506095504213](imgs/image-20230506095504213.png)
...
![image-20230506095634842](imgs/image-20230506095634842.png)

**运行时常量池**

类文件常量池是 *.class 文件中的，当该类被加载，它的常量池信息就会放入运行时常量池，并把里面的符号地址变为真实地址

![image-20230506100142724](imgs/image-20230506100142724.png)
...
![image-20230506100501905](imgs/image-20230506100501905.png)
...
![image-20230506100548455](imgs/image-20230506100548455.png)

下图是NIO传输数据的流程，在这个里面主要使用到了一个直接内存，不需要在堆中开辟空间进行数据的拷贝，jvm可以直接操作直接内存，从而使数据读写传输更快。

![image-20230506100621146](imgs/image-20230506100621146.png)
...
![](imgs/image-20220903233627146.png)
...
- **启动类加载器(BootStrap ClassLoader)：**
  ...
  不继承ClassLoader类，由C++编写实现。用于加载**JDK核心类库**（JAVA_HOME/jre/lib目录下）的类。

- **扩展类加载器(ExtClassLoader)：**

  ClassLoader的子类，主要加载加载**JDK扩展目录**中（JAVA_HOME/jre/lib/ext目录下）的类。

- **应用类加载器(AppClassLoader)：**

  ClassLoader的子类，主要用于加载**用户类路径**（classpath）下的类。也就是加载开发者自己项目中编写的类，以及第三方依赖的 jar 包。
...
![image-20230506100746624](imgs/image-20230506100746624.png)
...
如果一个类加载器在接到加载类的请求时，它首先不会自己尝试去加载这个类，而是把这个请求任务委托给上级类加载器去完成，依次递归，如果上级类加载器可以完成类加载任务，就返回成功；只有上级类加载器无法完成此加载任务时，才由下一级去加载。 

![image-20230506100920042](imgs/image-20230506100920042.png)
...
![image-20220903144547378](imgs/image-20220903144547378.png)
...
类从被加载到虚拟机中开始，直到被卸载，其生命周期大致分为：**加载、验证、准备、解析、初始化和卸载**六个阶段。其中，**验证、准备和解析统称为连接（Linking）阶段**。有时也会笼统地提及“使用”阶段，指类在程序中的实际运行和应用，但这并非 JVM 规范所正式定义的生命周期阶段。

![image-20230506101032605](imgs/image-20230506101032605.png)

**加载**

![image-20230506101115674](imgs/image-20230506101115674.png) 

- 通过类的全限定名，从文件系统或网络中获取对应的 `.class` 字节流；
- 将字节流解析为 JVM 运行时数据结构（类的元数据），存储在方法区中；
- 在堆内存中创建一个 `java.lang.Class` 实例，作为类的访问入口，用于 Java 层面对该类进行反射、实例化等操作。

![image-20230506101213373](imgs/image-20230506101213373.png)

**验证**

![image-20250528213727155](imgs/image-20250528213727155.png)
...
![image-20230506101445898](imgs/image-20230506101445898.png)
...
![image-20230506101824622](imgs/image-20230506101824622.png)

**解析**

![image-20230506101504632](imgs/image-20230506101504632.png)

把类中的符号引用转换为直接引用

比如：方法中调用了其他方法，方法名可以理解为符号引用，而直接引用就是使用指针直接指向方法。

![image-20230506102311951](imgs/image-20230506102311951.png)

**初始化**

![image-20230506101625087](imgs/image-20230506101625087.png)
...
![image-20230506101641837](imgs/image-20230506101641837.png)
...
![image-20230506104954777](imgs/image-20230506104954777.png)
...
![image-20230506111102825](imgs/image-20230506111102825.png)

```java
String demo = null;
```

![image-20230506111136231](imgs/image-20230506111136231.png)

当对象间出现了循环引用的话，则引用计数法就会失效

![image-20230506111255401](imgs/image-20230506111255401.png)

先执行右侧代码的前4行代码

![image-20230506111327590](imgs/image-20230506111327590.png)

目前上方的引用关系和计数都是没问题的，但是，如果代码继续往下执行，如下图

![image-20230506111512450](imgs/image-20230506111512450.png)
...
![image-20220904010634153](imgs/image-20220904010634153.png)
...
![image-20230506112047190](imgs/image-20230506112047190.png)
...
![image-20230506111919008](imgs/image-20230506111919008.png)
...
![image-20230506111957793](imgs/image-20230506111957793.png)
...
![image-20230506131229649](imgs/image-20230506131229649.png)
...
![image-20230506131308654](imgs/image-20230506131308654.png)

- 新创建的对象，都会先分配到eden区

![image-20230506131415418](imgs/image-20230506131415418.png)

- 当伊甸园内存不足，标记伊甸园与 from（现阶段没有）的存活对象
- 将存活对象采用复制算法复制到 to 中，复制完毕后，伊甸园和 from 内存都得到释放
- from与to是相对的，当s1为from，s2即为to，此时s1中可能有部分存活对象，s2中没有存活对象，在下一次垃圾回收时会将eden和from中的被标记的存活对象复制到to中，复制完，s1变成to，s2变成from。

![image-20230506131442503](imgs/image-20230506131442503.png)

- 经过一段时间后伊甸园的内存又出现不足，标记eden区域to区存活的对象，将存活的对象复制到from区

![image-20230506131544447](imgs/image-20230506131544447.png)

![image-20230506131607645](imgs/image-20230506131607645.png)
...
![image-20230506154006266](imgs/image-20230506154006266.png)
...
![image-20230506154042673](imgs/image-20230506154042673.png)
...
![image-20230506154323950](imgs/image-20230506154323950.png)

**Young Collection(年轻代垃圾回收)**

- 初始时，所有区域都处于空闲状态

  ![image-20230506154542687](imgs/image-20230506154542687.png)

- 创建了一些对象，挑出一些空闲区域作为伊甸园区存储这些对象

  ![image-20230506154607558](imgs/image-20230506154607558.png)

- 当伊甸园需要垃圾回收时，挑出一个空闲区域作为幸存区，用复制算法复制存活对象，需要暂停用户线程

  ![image-20230506154633118](imgs/image-20230506154633118.png)

  ![image-20230506154705088](imgs/image-20230506154705088.png)

- 随着时间流逝，伊甸园的内存又有不足

- 将伊甸园以及之前幸存区中的存活对象，采用复制算法，复制到新的幸存区，其中较老对象晋升至老年代

  ![image-20230506154759809](imgs/image-20230506154759809.png)

  ![image-20230506154826981](imgs/image-20230506154826981.png)

  ![image-20230506154859985](imgs/image-20230506154859985.png)
...
![image-20230506155000503](imgs/image-20230506155000503.png)

- 并发标记之后，会有重新标记阶段解决漏标问题，此时需要暂停用户线程。

- 这些都完成后就知道了老年代有哪些存活对象，随后进入混合收集阶段。此时不会对所有老年代区域进行回收，而是根据暂停时间目标优先回收价值高（存活对象少）的区域（这也是 Gabage First 名称的由来）。

  ![image-20230506155047765](imgs/image-20230506155047765.png)
...
![image-20230506155116267](imgs/image-20230506155116267.png)

其中H叫做巨型对象，如果对象非常大，会开辟一块连续的空间存储巨型对象

![image-20230506155146370](imgs/image-20230506155146370.png)
...
![image-20230506155341703](imgs/image-20230506155341703.png)
...
![image-20230506155416293](imgs/image-20230506155416293.png)
...
![image-20230506155501557](imgs/image-20230506155501557.png)
...
![image-20230506155518510](imgs/image-20230506155518510.png)

![image-20230506155552693](imgs/image-20230506155552693.png)
...
![image-20220904151948778](imgs/image-20220904151948778.png)
...
设置年轻代中Eden区和两个Survivor区的大小比例。该值如果不设置，则默认比例为8:1:1。Java官方通过增大Eden区的大小，来减少YGC发生的次数，但有时我们发现，虽然次数减少了，但Eden区满
...
![image-20220904104739581](imgs/image-20220904104739581.png)
...
![image-20220904111059602](imgs/image-20220904111059602.png)
...
![image-20220904114511854](imgs/image-20220904114511854.png)
...
![image-20220904115157363](imgs/image-20220904115157363.png)
...
![image-20220904115936095](imgs/image-20220904115936095.png)

可以内存、线程、类等信息

![image-20220904120057211](imgs/image-20220904120057211.png)
...
![image-20220904120356174](imgs/image-20220904120356174.png)

监控程序运行情况

![image-20220904132011289](imgs/image-20220904132011289.png)

查看运行中的dump

![image-20220904132134095](imgs/image-20220904132134095.png)

查看堆中的信息

![image-20220904132346495](imgs/image-20220904132346495.png)
...
![image-20230506155704119](imgs/image-20230506155704119.png)
...
![image-20220904132925812](imgs/image-20220904132925812.png)

通过查看堆信息的情况，可以大概定位内存溢出是哪行代码出了问题

![image-20220904133722905](imgs/image-20220904133722905.png)
...
![image-20220904161818255](imgs/image-20220904161818255.png)
...
![image-20220904162117022](imgs/image-20220904162117022.png)
...
>`printf "%x\n" 30979`
>
>![image-20220904162654928](imgs/image-20220904162654928.png)
...
![image-20220904162941977](imgs/image-20220904162941977.png)
