...
![img](imgs/1718937850264-c044349a-537e-4eca-8fe5-7002c1fa3c36.webp)
...
- 但是在 Redis 3.2 中，List 数据类型底层数据结构就只由 quicklist 实现了，替代了压缩列表和双向链表
...
![img](imgs/9fa26a74965efbf0f56b707a03bb9b7f.png)
...
![img](imgs/1719804939236-89f12a47-b851-4d06-a5f3-399e1119db57.png)
...
Redis中跳表的层高是在节点插入时，通过随机算法以固定概率（默认 25%）逐层晋升生成的，起始层为 1，每晋升一层需再次满足随机条件，直到失败为止或达到最大层数（默认 32 层），形成一种概率递减的层级结构，使得节点在底层数量多、在高层数量少，从而在空间与查询效率之间取得平衡，实现平均 O(log N) 的查找性能。
...
| 内存适配性      | ✅ 高（原生支持）   | ❌ 偏向磁盘型设计             |
| 插入/删除复杂度 | 简单，基于概率晋升 | 复杂，需频繁分裂与合并       |
| 有序遍历        | ✅ 原生支持         | ✅ 也支持（通过叶子节点链表） |
| 实现难度        | ✅ 简单，代码量少   | ❌ 复杂，结构维护难           |
| Redis 兼容性    | ✅ 非常适合         | ❌ 超出 Redis 设计哲学        |

> Redis 使用跳表而不是 B+ 树，主要是因为 Redis 是内存数据库，跳表结构更简单、适合内存操作，且支持高效的有序遍历和范围查询，非常契合 Sorted Set 的需求。相比之下，B+ 树虽然适合磁盘存储，查询效率高，但结构复杂、维护成本大，不适合 Redis 追求极致性能和低延迟的设计目标。
...
当执行`bgsave`时，会把共享内存标记为`read-only`（只读模式）。如果主进程执行读操作，则访问共享内存；如果主进程执行写操作时，则会拷贝一份数据，执行写操作，一旦拷贝数据之后，主进程在读数据时，读的也是新的数据了，页表的映射关系，也会映射到新的数据中去。

当然子进程读的依然是原来的数据。当子进程快照写完，子进程退出时，操作系统回收子进程占用的旧只读页，父进程继续使用自己的新页。
